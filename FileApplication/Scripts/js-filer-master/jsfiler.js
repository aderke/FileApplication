...
//    nameValidate: /^[^\\\/?%*:|'\"<>]+$/, /* Windows-like test pattern */
  ...
  var treeCommands = {/* core set */
    ...
    add: { label: 'Upload', icon: ['c_add.png', 'c_add-no.png']},
    ren: { label: 'Rename', icon: ['c_ren.png', 'c_ren-no.png']},
    del: { label: 'Delete', icon: ['c_del.png', 'c_del-no.png'] },
    dwn: { label: 'Download', icon: ['c_dwn.png', 'c_dwn-no.png'] }
  };
  $.jsfiler = {
    /** 
    ...
    Command: function(cmd, obj, prm) {
      ...
      if (cmd === 'sch') {
        ...
        $(tree.Id).jstree('rename_node', obj, prm.text);
          ...
          var fileNode = $(tree.Id).jstree('get_node', obj);

          var downloadLink = pg.linkDownloadFile +
           '?id=' + fileNode.id;

          var $downloadButton = $('a._dwn');
          $downloadButton.attr('href', downloadLink);
          $downloadButton[0].click();
          $downloadButton.attr('href', "#dwn");
       } else if (cmd === 'add') {
           var uploadNode = $(tree.Id).jstree('get_node', obj);
           $('span#folderToUpload').text(uploadNode.text);
          console.log(uploadNode);
           $('input#id').val(uploadNode.id);
           var $modalUploadBtn = $('a._add');
           $modalUploadBtn.attr('href', '#openModal');
           $modalUploadBtn[0].click();
           $modalUploadBtn.attr('href', '#add');
...
//        txt.push(NodeName(obj));
  ...
  var CheckReplace = function (cmd, node, parent) {
    ...
    //var fnameext = nodes[0].text;
    //var fname = fnameext.substring(0, fnameext.lastIndexOf('.'));
    //var ext = fnameext.substring(fnameext.lastIndexOf('.'), fnameext.length);
    //nodes[0].text = fname + " - Copy" + ext;
  ...
  var AllowCommands = function(obj) {
    ...
    for (var cmd in tree.cmdList) {
      if (
              ...
              (cmd === 'cpy' && node.type !== 0) ||
              (cmd === 'dwn' && node.type === 3 ||
              (cmd === 'cut' && node.type !== 2 && RemoveAllowed(node)) ||
